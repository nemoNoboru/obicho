...
fn create_bet(mut cx: FunctionContext) -> JsResult<JsString> {
    ...
    let bet = extract::get_bet_from_text(raw_text, user_id);
    match bet {
...
fn run_callback(results: String, callback: Root<JsFunction>, channel: Channel) {
    channel.send(move |mut cx| {
        let callback = callback.into_inner(&mut cx);
        let undefined = cx.undefined();
        let animals = enums::get_animal_from_number(results.parse().unwrap());
        let results = vec![cx.string(results), cx.string(format!("{:?}", animals))];
